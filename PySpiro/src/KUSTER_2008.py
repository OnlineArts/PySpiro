...
class KUSTER_2008(Reference):
    ...
    def __load_lookup_table(self) -> tuple: # No such file exists in this case
        pass
    ...
    def percent(self, sex: int, age: float, height: float, ethnicity: int, parameter: int, value: float, silent = True):
        ...
        if sex == 0: # Female
            if age < 18 or age > 80:
                ...
                if not silent:
                    print("Warning: Age must be between 18 and 80 years.")
                return pd.NA
            else:
                if height < 130 or height > 190:
                    # raise ValueError("Height must be between 130 and 190 cm, in females.")
                    if not silent:
                        print("Warning: Height must be between 130 and 190 cm, in females.")
                    return pd.NA
                else:
                    if parameter == 1:
            ...
            if age < 18 or age > 80:
                ...
                if height < 140 or height > 200:
                    # raise ValueError("Height must be between 140 and 200 cm, in males.")
                    if not silent:
                        print("Warning: Height must be between 140 and 200 cm, in males.")
                    return pd.NA
                else:
                    if parameter == 1:
